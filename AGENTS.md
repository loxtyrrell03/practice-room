...
Repertoire offers Dates beside every piece and All performances & deadlines above the timeline. One editor handles shared events, any selected movements, exact dates or provisional months, and low/medium/high importance. Existing and new events without an explicit importance default to medium. Past dates and removed events stay collapsed in the manager; removed events can be restored. Settings links to the same manager rather than maintaining a second date-editing path. Timeline lanes identify each event and movement scope; importance is shown in the performance manager/editor; the overview identifies the next deadline, and month filters derive from actual dates. Editors retain their input on failures; repeat submissions retain request identity. A mutation requests a fresh snapshot after an already-running poll, preventing an older read from hiding a just-saved date. The mobile editor uses the existing touch/keyboard help and scrollable dialog design. No new top-level navigation or theme was introduced.
...
### Current urgency versus event importance

Importance is an event setting, not a current practice rank. The earlier additive importance floor could make a distant high-importance recital outweigh a substantially nearer medium assessment. Deadline pressure now applies importance only to a bounded urgency curve with a neutral baseline; urgency increases smoothly as the date approaches. Tests compare nearer medium and later high events across several points in the term and check actual allocated minutes, plus equal-date importance and post-event continuity. UI labels say importance only in the manager/editor, while the overview marks the next deadline. Existing event settings, workload weights, movement scope and practice evidence remain intact. Asset version 42 carries this correction.


Version 42 is deployed on the existing phone route. The connected rendered overview identifies the nearer assessment as the next deadline and no longer presents static importance as a current practice rank. The urgency correction passed 62 focused Python tests, 20 frontend checks and the gateway test, including a real allocation comparison with equal workloads. Deployment preserved existing routes and unrelated services. Booking freshness guards remain authoritative: a stale agenda retains its previous plan until fresh booking evidence arrives.
